...
def create_app(test_config=None):
    ...
    def get_all_categories():
        ...
        categories = [category.format() for category in query_categories]
    ...
    def question_search():
        ...
        try:
            selection_questions = Question.query.order_by(Question.id).filter(
            ...
            return jsonify(
                {
                    "success": True,
                    "questions": current_questions,
                    "total_questions": len(selection_questions.all()),
                }
            )
    ...
    def get_questions_by_category(category_id):
        try:
            query_questions = Question.query.filter_by(category=category_id).all()
    ...
    def quizzes():
        ...
        if quiz_category['type'] == 'click':
            ...
            query_questions = Question.query.filter_by(category=quiz_category['id']).all()
        ...
        if len(next_questions) > 0:
            selected_question = random.choice(next_questions)
